...
| ADR-008 | Synthetic catalog coverage is intentionally narrow | Document the gap; don't regenerate to make demos look better |
...
| ADR-010 | Multilingual embedding limits on abstract compound German nouns | Document the limit; do not lower threshold for German queries |
...
| ADR-016 | BigQuery vector store implementation deferred | Practical experience gap; stub captures design without overclaiming expertise |
...
**Context:**
We needed an LLM provider for intent classification, clarification, and query expansion. PAYBACK runs on GCP and uses Gemini in production; the case study references both Claude and Gemini as plausible choices.
...
**Rationale:**
A second LLM implementation would have consumed 2+ hours of SDK setup and provider-specific output-format testing without unlocking new system behavior. The architectural signal that matters is the *interface*: `IntentAgent`, the router, the expander, and the clarification agent all accept any `LLMClient`. Adding a provider is one new class, no caller changes. The Anthropic unified endpoint also gave a cost and quota advantage for the development phase. This is "extensibility by design, not by implementation."

**Consequences:**
At the onsite, defensible as: *"I built the interface and one implementation; a second is one new class away."* The `judge_client.py` Sonnet/Opus split (ADR-017) is a degenerate use of the same pattern within one provider. Cross-provider judge eval would further reduce bias (ADR-013).
...
**Rationale:**
This model is widely benchmarked for German, runs locally with no GCP dependency during development, loads in under 2 seconds, and is small enough for fast iteration. The 768-dim alternative (`paraphrase-multilingual-mpnet-base-v2`) was considered but deferred until limits surfaced in practice. The regression test turns a capability claim into a verifiable assertion — cross-lingual retrieval is enforced, not hoped-for. Limits discovered later are documented in ADR-010.
...
**Context:**
We need a vector store for ~700 products in development, and a clear path to the production backend. PAYBACK operates a BigQuery footprint; co-locating embeddings there is the natural production architecture.
...
**Consequences:**
Dev and demo ship with ChromaDB. Production migration is a configuration change (`VECTOR_STORE=bigquery`) plus a backfill ingest — no code refactor. The stub's schema and query pattern are sufficient for a production engineer to implement without starting from scratch.
...
**Context:**
Users may mention PAYBACK partners we don't carry. "Pasta from REWE" should not fail with an error or pretend to filter on a partner we can't serve.
...
**Consequences:**
Users mentioning out-of-scope partners get useful results from our catalog. This pattern scales — PAYBACK has hundreds of real partners; the same fallthrough handles any unrecognized name without code changes.
...
**Context:**
Observed during testing: "Schokolade" returns chocolate-flavored yogurts and ice creams — no chocolate bars in the catalog. "Stuff for pasta dinner" returns pasta, olive oil, and canned tomatoes from EDEKA only, because dm and Amazon don't stock grocery items in the synthetic data. Pre-flight category discovery surfaces these gaps honestly.

**Decision:**
Keep the catalog as-is. Document the limit. Do not regenerate to make demo queries look better.
...
**Consequences:**
Pre-flight adds ~50ms (a ChromaDB query). The dropped-queries list is honest signal about catalog gaps. At the onsite, this is the clearest example of "we observed reality not matching design and fixed it with grounding, not prompt tuning."
...
**Decision:**
Document the limit. Do not lower the relevance threshold for German queries.
...
**Decision:**
Implement the ranker as a three-signal weighted sum: `final_score = α·semantic + β·commercial + γ·diversity` with defaults α=0.6, β=0.3, γ=0.1 (all configurable via env). Commercial boost combines `points_multiplier` and `active_promo`. Diversity bonus has two modes: known user (`(1 - affinity[partner]) * scale`) and cold-start (`1 - (partner_count_in_topk / topk)`).

**Rationale:**
Three signals, each independently defensible and bounded to [0, 1], so the composite score is also bounded. The cold-start path uses the same equation shape with a different input — result-set partner distribution — avoiding special-case branching logic. Weights are read from `settings` on each call, making them tunable via env vars without redeployment. Alternative considered: a learning-to-rank model. Rejected as over-engineering for a 700-product catalog with synthetic user data.
...
**Context:**
Query expansion adds latency (~1-2s extra for LLM call + per-sub-query retrievals) and cost. It should fire only when a query implies multiple related products worth expanding. The question was how to detect that.
...
**Consequences:**
Support queries cost one LLM call (intent classification only) and respond in ~3-5s. The hardcoded message is easy to update when the support URL changes. The branch is tested in `test_support_intent_returns_out_of_scope_message`.
...
**Context:**
The case study explicitly suggests BigQuery as the production vector backend. PAYBACK operates a large BigQuery footprint; co-locating embeddings there avoids a separate vector database and enables native joins with purchase history and user signal data.
...
**Consequences:**
The demo ships with ChromaDB. Production migration is a configuration change (`VECTOR_STORE=bigquery`) plus a backfill ingest — no code refactor. At the onsite, this is an honest answer: *"I have not deployed BigQuery vector search in production. I designed the system so that I could."*
...
**Decision:**
The production system (intent agent, clarification agent, query expander) uses Claude Sonnet (`claude-sonnet-4-6`). The eval judge uses Claude Opus (`claude-opus-4-7`). These are implemented as separate clients: `ClaudeClient` for production, `OpusJudgeClient` for evaluation.
...
**Consequences:**
Eval runs cost more (Opus is priced higher than Sonnet) but eval frequency is low. The limitation — correlated biases within Anthropic's model family — is acknowledged in the `judge_client.py` docstring and in ADR-013. Production would add a cross-provider judge on top of this split.
